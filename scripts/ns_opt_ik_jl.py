#!/usr/bin/env python3
"""
Pose IK with DLS (world-frame target) — trajectory to target + hold + null-space joint-limit avoidance.
...
- Adds a null-space secondary task that pulls joints toward mid-range to avoid limits.
...
NS_GAIN                 = 0.8          # <— tune this (0.2–2.0 typical)
NS_STRONG_NEAR_LIMITS   = True         # amplify push when near limits
NS_EPS                  = 1e-3         # small for numerical safety in weights
...
# =========================
# Joint-limit avoidance helpers
# =========================
def joint_limits_arrays(model, joint_ids):
    q_lo = np.array([model.jnt_range[jid][0] for jid in joint_ids])
    q_hi = np.array([model.jnt_range[jid][1] for jid in joint_ids])
    q_mid = 0.5 * (q_lo + q_hi)
    r = 0.5 * (q_hi - q_lo)           # half-range per joint
    return q_lo, q_hi, q_mid, r

def nullspace_projector(J, J_pinv):
    # 7x7 for a 7-DoF arm: N = I - J^† J
    n = J_pinv.shape[0]
    return np.eye(n) - J_pinv @ J

def joint_limit_ns_des(q, q_mid, r):
    """
    Desired null-space velocity toward mid-range with optional near-limit boosting.
    Base direction: (q_mid - q) / r^2  (units: 1/rad)
    """
    base = (q_mid - q) / (r**2)

    if not NS_STRONG_NEAR_LIMITS:
        return NS_GAIN * base

    # Boost near limits:
    # delta = normalized displacement from mid in [-1,1]; boost ~ 1 / (eps + (1 - delta^2))
    delta = (q - q_mid) / r
    boost = 1.0 / (NS_EPS + (1.0 - np.clip(delta**2, 0.0, 1.0)))
    return NS_GAIN * boost * base
...
# =========================
# Main loop (IK + trajectory + null-space)
# =========================
def run():
    ...
    # Joint limits info for null-space task
    q_lo, q_hi, q_mid, r = joint_limits_arrays(model, joint_ids)
    ...
    print("\n=== Pose IK (trajectory -> hold) with Null-space Joint-limit Avoidance ===")
    ...
    print(f"NS_GAIN = {NS_GAIN}, strong_near_limits = {NS_STRONG_NEAR_LIMITS}\n")
    ...
    with viewer.launch_passive(model, data, show_left_ui=False, show_right_ui=False) as v:
        while v.is_running():
            ...
            # Log first convergence to the moving/final target
            ...
            qdot_ns_des = joint_limit_ns_des(q_ref, q_mid, r) # (7,)
